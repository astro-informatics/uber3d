...
template <typename T> class normalAlloc__
  {
  public:
    static T *alloc(uint64_t sz) { return (sz>0) ? new T[sz] : 0; }
  ...
  public:
    static T *alloc(uint64_t sz)
      ...
      if (align<=max_nat_align)
        ...
        {
        uint64_t overhead=align-1+sizeof(void*);
        void *ptr=malloc(sz*sizeof(T)+overhead);
        planck_assert(ptr!=0,"error in malloc()");
        uint64_t sptr=reinterpret_cast<uint64_t>(ptr);
...
template <typename T> class arr_ref
  ...
  public:
    ...
    arr_ref(T *d_, uint64_t s_) : s(s_),d(d_) {}

    /*! Returns the current array size. */
    uint64_t size() const { return s; }

    /*! Writes \a val into every element of the array. */
    void fill (const T &val)
      { for (uint64_t m=0; m<s; ++m) d[m]=val; }
    ...
    template<typename T2> void copyToPtr (T *ptr) const
      { for (uint64_t m=0; m<s; ++m) ptr[m]=d[m]; }
    ...
    uint64_t find (const T &val) const
      {
      for (uint64_t m=0; m<s; ++m)
        if (d[m]==val) return m;
...
template <typename T, uint64_t sz> class fix_arr
  {
  private:
    T d[sz];
...
template <typename T, typename stm> class arrT: public arr_ref<T>
  ...
  public:
    ...
    explicit arrT(uint64_t sz) : arr_ref<T>(stm::alloc(sz),sz), own(true) {}
    /*! Creates an array with \a sz entries, and initializes them with
        \a inival. */
    arrT(uint64_t sz, const T &inival) : arr_ref<T>(stm::alloc(sz),sz), own(true)
      { this->fill(inival); }
    ...
    arrT (T *ptr, uint64_t sz): arr_ref<T>(ptr,sz), own(false) {}
    ...
    arrT (const arrT &orig): arr_ref<T>(stm::alloc(orig.s),orig.s), own(true)
      { for (uint64_t m=0; m<this->s; ++m) this->d[m] = orig.d[m]; }
    ...
    void allocAndFill (uint64_t sz, const T &inival)
    ...
    void resize (uint64_t sz)
      ...
      if (sz==this->s) return;
      T *tmp = stm::alloc(sz);
      for (uint64_t m=0; m<min(sz,this->s); ++m)
        tmp[m]=this->d[m];
      if (own) stm::dealloc(this->d);
      this->s = sz;
      this->d = tmp;
    ...
    template<typename T2> void copyFrom (const std::vector<T2> &orig)
      {
      alloc (orig.size());
      for (uint64_t m=0; m<this->s; ++m) this->d[m] = orig[m];
      }
    ...
    template<typename T2> void copyTo (std::vector<T2> &vec) const
      ...
      for (uint64_t m=0; m<this->s; ++m) vec.push_back(this->d[m]);
      }

    /*! Reserves space for \a sz elements, then copies \a sz elements
        from \a ptr into the array. */
    template<typename T2> void copyFromPtr (const T2 *ptr, uint64_t sz)
      {
      alloc(sz);
      for (uint64_t m=0; m<this->s; ++m) this->d[m]=ptr[m];
      }
...
template <typename T>
  ...
  public:
    ...
    /*! Creates an array with \a sz entries. */
    explicit arr(uint64_t sz) : arrT<T,normalAlloc__<T> >(sz) {}
    /*! Creates an array with \a sz entries, and initializes them with
        \a inival. */
    arr(uint64_t sz, const T &inival) : arrT<T,normalAlloc__<T> >(sz,inival) {}
    /*! Creates an array with \a sz entries, which uses the memory pointed
        to by \a ptr.
        \note \a ptr will <i>not</i> be deallocated by the destructor.
        \warning Only use this if you REALLY know what you are doing.
        ...
        Other restrictions may apply. You have been warned. */
    arr (T *ptr, uint64_t sz): arrT<T,normalAlloc__<T> >(ptr,sz) {}
    /*! Creates an array which is a copy of \a orig. The data in \a orig
        is duplicated. */
...
template <typename T, int align>
  ...
  public:
    ...
    /*! Creates an array with \a sz entries. */
    explicit arr_align(uint64_t sz) : arrT<T,alignAlloc__<T,align> >(sz) {}
    /*! Creates an array with \a sz entries, and initializes them with
        \a inival. */
    arr_align(uint64_t sz, const T &inival)
...
template <typename T, typename storageManager> class arr2T
  ...
  public:
    ...
    arr2T(uint64_t sz1, uint64_t sz2)
    ...
    /*! Creates an array with the dimensions \a sz1 and \a sz2 from existing
        pointer. */
    arr2T(T* p, uint64_t sz1, uint64_t sz2)
      : s1(sz1), s2(sz2), d(p, s1*s2) {}
    arr2T(uint64_t sz1, uint64_t sz2, const T &inival)
      : s1(sz1), s2(sz2), d (s1*s2)
      { fill(inival); }
    ...
    void alloc (uint64_t sz1, uint64_t sz2)
      {
      if (sz1*sz2 != d.size())
        d.alloc(sz1*sz2);
      s1=sz1; s2=sz2;
    ...
    /*! Allocates space for an array with \a sz1*sz2 elements.
        ...
        \a sz1 or \a sz2 can be 0. If \a sz1*sz2 is the same as the
        currently allocated space, no reallocation is performed. */
    void allocAndFill (uint64_t sz1, uint64_t sz2, const T &inival)
      { alloc(sz1,sz2); fill(inival); }
    /*! Allocates space for an array with \a sz1*sz2 elements.
        The content of the array is undefined on exit.
        \a sz1 or \a sz2 can be 0. If \a sz1*sz2 is smaller than the
        currently allocated space, no reallocation is performed. */
    void fast_alloc (uint64_t sz1, uint64_t sz2)
      {
      if (sz1*sz2<=d.size())
        { s1=sz1; s2=sz2; }
      else
    ...
    void fill (const T &val)
      { for (uint64_t m=0; m<s1*s2; ++m) d[m]=val; }
...
template <typename T>
  ...
  public:
    ...
    /*! Creates an array with the dimensions \a sz1 and \a sz2. */
    arr2(uint64_t sz1, uint64_t sz2) : arr2T<T,normalAlloc__<T> > (sz1,sz2) {}
    /*! Creates an array with the dimensions \a sz1 and \a sz2 from existing
        pointer. */
    arr2(T* p, uint64_t sz1, uint64_t sz2) : arr2T<T,normalAlloc__<T> > (p,sz1,sz2) {}
    /*! Creates an array with the dimensions  \a sz1 and \a sz2
        and initializes them with \a inival. */
    arr2(uint64_t sz1, uint64_t sz2, const T &inival)
...
template <typename T, int align>
  ...
  public:
    ...
    /*! Creates an array with the dimensions \a sz1 and \a sz2. */
    arr2_align(uint64_t sz1, uint64_t sz2)
      : arr2T<T,alignAlloc__<T,align> > (sz1,sz2) {}
    /*! Creates an array with the dimensions  \a sz1 and \a sz2
        and initializes them with \a inival. */
    arr2_align(uint64_t sz1, uint64_t sz2, const T &inival)
...
template <typename T> class arr2b
  ...
  private:
    ...
    void fill_d1()
      { for (uint64_t m=0; m<s1; ++m) d1[m] = &d[m*s2]; }

  public:
    /*! Creates a zero-sized array. */
    arr2b() : s1(0), s2(0), d(0), d1(0) {}
    /*! Creates an array with the dimensions \a sz1 and \a sz2. */
    arr2b(uint64_t sz1, uint64_t sz2)
    ...
    arr2b &operator= (const arr2b &orig)
      ...
      if (this==&orig) return *this;
      alloc (orig.s1, orig.s2);
      for (uint64_t m=0; m<s1*s2; ++m) d[m] = orig.d[m];
      return *this;
      }
...
template <typename T> class arr3
  ...
  public:
    ...
    arr3(uint64_t sz1, uint64_t sz2, uint64_t sz3)
